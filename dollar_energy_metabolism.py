...
This is a geometric series. If the recursive recycling
...
Core equation (two-parameter model):

    subtotal = E_base + E_base × Σ r_i
    E_total  = subtotal / (1 - r_recursive)

where:
    E_base       = primary-energy intensity of $1 GDP (MJ/$)
    r_i          = per-layer direct overhead ratio (low, mid, high band)
    r_recursive  = tail recycling fraction for secondary activity
                   generated by the first-pass overhead itself

If r_recursive >= 1 the geometric tail diverges: the financial
system's own recursion consumes the project's energy budget.
This is the negative-EROI condition.
...
# The five overhead layers, empirically bounded
OVERHEAD_LAYERS = [
    OverheadLayer(
        name="leverage",
        r_low=0.11,
...
class Scenario:
    ...
    recursive_r: float          # tail recycling fraction for geometric series
    description: str
...
def compute_dollar_energy(scenario: Scenario) -> dict:
    ...
    Two-parameter model:
        subtotal = E_base + Σ (r_i × E_base)    # one pass of direct layers
        E_total  = subtotal / (1 - r_recursive) # geometric tail
...
def compute_project_audit(project: ClimateProject, scenario: Scenario) -> dict:
    ...
    if claimed > 0 and not math.isinf(results["annual_budget_low"]["CO2_tonnes"]):
        funding_frac_low = results["annual_budget_low"]["CO2_tonnes"] / claimed
        funding_frac_high = results["annual_budget_high"]["CO2_tonnes"] / claimed
    else:
        funding_frac_low = float('inf')
        funding_frac_high = float('inf')
    ...
    return {
        ...
        "funding_CO2_as_fraction_of_claimed": {
            "low_budget": funding_frac_low,
            "high_budget": funding_frac_high,
        },
...
def find_breakeven_r(project: ClimateProject, E_base: float = 6.0,
    ...
    """
    Find the recursive recycling fraction r at which the funding's
    own CO2 emissions equal the project's claimed annual sequestration.

    Defaults (E_base=6.0, subtotal=13.7) correspond to the
    "typical climate finance" scenario mid-layer subtotal. Pass explicit
    values to query breakeven against other scenario baselines.

    If claimed == 0 (e.g., SAI), returns 0.0 (always net negative).
    ...
    #   r_breakeven = 1 - (annual_USD * subtotal * CO2_per_MJ_kg / target)
